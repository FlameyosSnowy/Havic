...
public abstract class AbstractCommandManager<P, C extends ContextCommand<S>, S extends Sender> {
    ...
    private final List<C> commands = new ArrayList<>(10);
    ...
    public final void register(C @NotNull ... commands) {
        for (C command : commands) register(command);
    }
    ...
    public void setDurationNames(@NotNull List<String> names) {
        ...
        if (size != namesSize) throw new IllegalArgumentException("Size should have been " + namesSize + " but turned out to be " + size);
