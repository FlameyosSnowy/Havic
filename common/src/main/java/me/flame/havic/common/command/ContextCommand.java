...
public abstract class ContextCommand<S extends Sender> {
    ...
    private final Lazy<Map<String, SubcommandHandler<S>>> container = new Lazy<>(new HashMap<>(5));
     ...
     * @return         success rate
     */
    public abstract Response execute(S sender, Arguments args);
     ...
     */
    protected final <C extends SubcommandHandler<S>> void registerSubCommand(String string, C handler) {
     ...
     *
     * @see #registerSubCommand(String, SubcommandHandler)
     * @return unmodifiable map of all subcommands registered with this command
     */
    @NotNull
    @Unmodifiable
    public final Map<String, SubcommandHandler<S>> subcommands() {
        TreeMap
