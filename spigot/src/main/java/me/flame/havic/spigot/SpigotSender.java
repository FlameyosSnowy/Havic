...
public class SpigotSender implements Sender, Permissible, Conversable, Provide<SpigotSender, SpigotSender> {
    ...
    public @NotNull Player asPlayer() {
        return Preconditions.checkNotNull(getPlayer(), "Sender casted to Player but is not Player via #requirePlayer.");
    }

    @Override
    public @NotNull ConsoleCommandSender asConsole() {
        return  Preconditions.checkNotNull(getConsole(), "Sender casted to ConsoleCommandSender but is not ConsoleCommandSender via #requireConsole.");
    }
