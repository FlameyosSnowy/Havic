...
public final class NativeSpigotCommand extends Command {
    ...
    private static boolean executeAsync(final Map<String, CommandHandler<SpigotSender>> subcommands,
        ...
        CompletableFuture.runAsync(() -> {
            ...
            Map<String, CommandHandler<SpigotSender>> synchronizedSubcommands =
                    Collections.synchronizedMap(subcommands);

            checkSubCommand(player, command, synchronizedSubcommands, args);
        });
        return true;
    }

    private static void executeCommand(@NotNull final SpigotSender player,
                                       @NotNull final SpigotCommand command,
    ...
    private static boolean checkSubCommand(@NotNull final SpigotSender sender,
        ...
        if (subcommands.isEmpty() || arguments.isEmpty()) {
            executeCommand(sender, command, arguments);
            return true;
        }
