...
public final class NativeSpigotCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        final Map<String, SubcommandHandler<SpigotSender>> subcommands = command.subcommands();
        ...
        return (command.isAsync() || manager.executionType().isAsync())
                ...
                : executeCommand(player, command, arguments) ||
    ...
    private static boolean executeAsync(final Map<String, SubcommandHandler<SpigotSender>> subcommands,
                                        final SpigotCommand command,
                                        final SpigotSender player,
                                        final Arguments args) {
        CompletableFuture.runAsync(() -> {
            // must synchronize for thread-safety.
            Map<String, SubcommandHandler<SpigotSender>> synchronizedSubcommands = Collections.synchronizedMap(subcommands);

            if (checkSubCommand(player, command, synchronizedSubcommands, args)) return;
            executeCommand(player, command, args);
        });
        return true;
    }

    private static boolean executeCommand(@NotNull final SpigotSender player,
                                          @NotNull final SpigotCommand command,
        ...
        if (response == Response.USAGE) {
            player.send(Message.CORRECT_USAGE.getMessage() + command.getUsage());
        }
        return true;
    }

    private static boolean checkSubCommand(@NotNull final SpigotSender sender,
                                           @NotNull final SpigotCommand command,
                                           @NotNull final Map<String, SubcommandHandler<SpigotSender>> subcommands,
                                           @NotNull final Arguments arguments) {
        ...
        subcommand.handle(sender, arguments.copy(subcommandArguments, 1));
        return true;
    }
}